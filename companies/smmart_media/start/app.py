...
WHATSAPP_TO_ASSISTANT = {
    "447464177761": "openai-assistant-id="asst_AS82w4Y1Nd6sSR8KhJbIodad",
    "447510698847": "openai-assistant-id-"asst_A2CqaZ1HO1qI6QwjyYIrmqeI",
    # Add more as needed
}
                    ...
                    for message in messages:
                        ...
                        if sender_number != business_number:
                            ...
                            if message_type == "text":
                                ...
                                assistant_id = WHATSAPP_TO_ASSISTANT.get(business_number)
                                response = generate_response("I can see you sent an image. How can I help you with it?", sender_number, sender_name, assistant_id=assistant_id)
                                send_whatsapp_message(sender_number, response, business_number)
                                ...
                                assistant_id = WHATSAPP_TO_ASSISTANT.get(business_number)
                                response = generate_response("I received your audio message. Could you please send a text message instead?", sender_number, sender_name, assistant_id=assistant_id)
                                send_whatsapp_message(sender_number, response, business_number)
                                ...
                                assistant_id = WHATSAPP_TO_ASSISTANT.get(business_number)
                                response = generate_response("I can see you sent a document. How can I help you with it?", sender_number, sender_name, assistant_id=assistant_id)
                                send_whatsapp_message(sender_number, response, business_number)
                                
                            else:
                                logger.info(f"Received unsupported message type: {message_type}")
                                assistant_id = WHATSAPP_TO_ASSISTANT.get(business_number)
                                response = generate_response("I received your message but I can only respond to text messages at the moment.", sender_number, sender_name, assistant_id=assistant_id)
                                send_whatsapp_message(sender_number, response, business_number)
                        else:
